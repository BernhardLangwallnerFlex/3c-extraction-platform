...
- **Redis outages:** `jobs.worker.build_redis_connection()` gives the client a `Retry(ExponentialBackoff(1, 30), retries=15)` policy — ~331s of backoff, enough to ride out a node reboot. This matters because RQ retries `redis.exceptions.ConnectionError` but treats the *sibling* `redis.exceptions.TimeoutError` as fatal ("Redis connection timeout, quitting..."), and redis-py's default is `retries=0`. Without the policy a single timeout exits the process and Container Apps crash-loops it — that is what turned a 2.5-minute outage on 2026-08-24 into 36 Sentry events. `connect_with_retry()` covers the same failure at cold start, where RQ is not yet in the loop. The cache was upgraded Basic C0 -> **Standard C0** on 2026-08-24 ($16.06 -> $40.15/mo), which adds the replica and the 99.9% SLA, so patching is now a seconds-long failover rather than a ~150s outage. C1 was considered and rejected: peak memory use is 0.95 MB of ~220 MB usable and peak connections 23 of 256, so the 1 GB was pointless — the SLA comes from the Standard tier, not the size. Note it is still **one cache shared by all 3 products across both tiers**, so any cache event is fleet-wide. The upgrade itself was fully non-disruptive: connectedclients held flat at 19 with zero worker restarts.
