...
- 🐞 **`SENTRY_DSN` empty is rejected by Azure** (`secret ... value or keyVaultUrl and identity should be provided`). The script defaults `SENTRY_DSN=""` and treats it as optional, but ACA won't create an empty secret. Worked around by reusing the **production** `sentry-dsn` (no separate `3cix-vetcostcheck` Sentry project exists yet — that's Task 16 Step 4). **Latent issue for `bps`/`sanierer`: the script as written required a non-empty `SENTRY_DSN`.** Hardened in a follow-up commit: the `sentry-dsn` secret + `SENTRY_DSN` env var are now wired only when a DSN is provided (the app disables Sentry when unset), using the bash-3.2-safe `${arr[@]+"${arr[@]}"}` empty-array idiom.
